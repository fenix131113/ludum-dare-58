...
namespace InventorySystem
{
    public class Inventory
    {
        ...
        public bool TryAddItem(Item item)
        {
            ...
            {
                var allMatches = _items.Where(x => x.Source == item.Source);

                foreach (var match in allMatches)
                {
                    var itemAdded = TryAddItemInOther(item, match, false);
            ...
            return item.Count == startItemCount;
        }
        ...
        public bool TryRemoveItem(ItemDataSO source, int count = 1)
        {
            ...
            var allMatches = _items.Where(x => x.Source == source && x.Count >= count);

            var enumerable = allMatches.ToList();

            if (enumerable.Sum(x => x.Count) < count)
                return false;
            ...
            foreach (var match in enumerable)
            {
                ...
                {
                    match.TryChangeCount(-(match.Count - count));
                    OnInventoryChanged?.Invoke();
                    return true;
                }
        ...
        private static bool TryAddItemInOther(Item item, Item other, bool addFull)
        {
            ...
            switch (addFull)
            {
                ...
                case false:
                    ...
                    if (item.Count <= need)
                    {
                        var add = item.Count - (need - item.Count);
